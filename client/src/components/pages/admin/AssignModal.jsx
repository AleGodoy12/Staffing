...
export default function AssignModal({
	...
	const assignEmployee = async (proyecto, empleado, horas) => {
		...
		const res = await axios.post(
			`${url}project/${project_id}/${employee_id}/${hours_to_assign}`
		);
		setOpen(false);
		setHoursToAssign(0);
		await load();
	...
	return (
		<>
			<button className="open-modal" onClick={openM}>
				Asignar
			</button>
			<section
				className="modal"
			...
			>
				<div className="modal-container">
					...
					<h3>{nameSurname}</h3>
					<p>
						Horas disponibles: <span>{freeHours}</span>
					</p>
					<p>
						Horas ocupadas: <span>{usedHours}</span>
					</p>
					<p>
						Horas totales: <span>{totalHours}</span>
					</p>
					<input
						value={hoursToAssign || ''}
					...
					<div>
						...
						<button
							onClick={() =>
								assignEmployee(idProject, idEmployee, hoursToAssign)
							}
						>
							{' '}
							Aceptar{' '}
